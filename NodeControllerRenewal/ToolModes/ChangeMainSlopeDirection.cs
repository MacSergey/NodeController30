...
namespace NodeController
{
    public class ChangeMainSlopeDirectionToolMode : NodeControllerToolMode
    {
        ...
        public override void RenderOverlay(RenderManager.CameraInfo cameraInfo)
        {
            ...
            if (!IsSelectedSegmentEnd)
            {
                Tool.Data.MainBezier.Render(new OverlayData(cameraInfo) { Width = width, Color = CommonColors.Yellow, RenderLimit = underground });

                foreach (var segmentData in Tool.Data.SegmentEndDatas)
                    segmentData.RenderCircle(new OverlayData(cameraInfo) { Color = segmentData.OverlayColor, RenderLimit = underground });

                foreach (var segmentData in Tool.Data.MainSegmentEndDatas)
                    segmentData.Position.RenderCircle(new OverlayData(cameraInfo) { Color = segmentData == HoverSegmentEnd ? Color.white : CommonColors.Yellow, RenderLimit = underground }, width, 0f);
            }
            else
            {
                ...
                SelectedSegmentEnd.Position.RenderCircle(new OverlayData(cameraInfo) { Color = CommonColors.Yellow, RenderLimit = underground }, width, 0f);

                if (IsHoverSegmentEnd)
                    HoverSegmentEnd.Position.RenderCircle(new OverlayData(cameraInfo) { Color = CommonColors.Yellow, RenderLimit = underground }, width, 0f);
                else
                    endPosition.RenderCircle(new OverlayData(cameraInfo) { Color = CommonColors.Yellow, RenderLimit = underground }, width, 0f);
            }
        }
    }
}
